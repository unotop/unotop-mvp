/*
 * Legacy snapshot stripped.
 * Original content captured in docs/spec-logic-v1.md.
 * File kept as placeholder only.
 */
export {};
  return Math.pow(1 + a, 1 / 12) - 1;
}
function fairRoundTo100(map: Record<string, number>) {
  const keys = Object.keys(map);
  if (keys.length === 0) return {};
  const sum = keys.reduce((a, k) => a + (map[k] || 0), 0) || 1;
  const scaled = keys.map((k) => ({ k, x: (map[k] || 0) * (100 / sum) }));
  const floors = scaled.map(({ k, x }) => ({
    k,
    v: Math.floor(x),
    r: x - Math.floor(x),
  }));
  let total = floors.reduce((a, o) => a + o.v, 0);
  const rest = 100 - total;
  floors.sort((a, b) => b.r - a.r);
  for (let i = 0; i < rest; i++) floors[i % floors.length].v += 1;
  const out: Record<string, number> = {};
  floors.forEach(({ k, v }) => (out[k] = v));
  return out;
}
...
const CASH_BASE_RETURN = 0.0;

function getAssetsByScenario(
  scn: "conservative" | "base" | "aggressive"
): AssetMap {
  const ETF =
    scn === "conservative" ? 0.09 : scn === "aggressive" ? 0.18 : 0.14;
  const ETF_RISK = scn === "conservative" ? 5 : scn === "aggressive" ? 6 : 5;

  const GOLD =
    scn === "conservative" ? 0.07 : scn === "aggressive" ? 0.11 : 0.095;
  const GOLD_RISK = scn === "aggressive" ? 3 : 2;

  const CRYPTO =
    scn === "conservative" ? 0.12 : scn === "aggressive" ? 0.35 : 0.2;
  const CRYPTO_RISK = scn === "aggressive" ? 9 : 9;

  const DYNAMIC_M =
    scn === "conservative" ? 0.02 : scn === "aggressive" ? 0.04 : 0.03;
  const DYNAMIC = annualizeFromMonthly(DYNAMIC_M);
  const DYNAMIC_RISK =
    scn === "conservative" ? 8 : scn === "aggressive" ? 9 : 9;
