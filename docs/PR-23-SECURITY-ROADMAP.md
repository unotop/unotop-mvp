  ...
  if (validated.mix) {
    ...
    validated.mix = validated.mix.map(item => {
      if (item.pct > MAX_LIMITS.individualAsset) {
        console.warn(`[Persist] Invalid ${item.key} percentage, capping at 100%`);
        return { ...item, pct: 100 };
      }
      if (item.pct < 0) {
        console.warn(`[Persist] Negative ${item.key} percentage, resetting to 0`);
        return { ...item, pct: 0 };
      }
      return item;
    });
...
const SECRET_KEY = import.meta.env.VITE_DEEPLINK_SECRET || 'default-dev-key-change-in-prod';
...
test('blocks 6th request within 1 hour', async () => {
  const requests = Array(6).fill(null).map(() => 
    fetch('/.netlify/functions/send-projection', { method: 'POST', body: validData })
  );
...
test('auto-corrects tampered localStorage', () => {
  localStorage.setItem('unotop:v3', JSON.stringify({
...
- [ ] Inspect bundle → no VITE_EMAILJS_* strings found
