...
import { canSubmit, recordSubmission, getRemainingSubmissions } from '../src/utils/rate-limiter';
...
describe('Security Features', () => {
  ...
  describe('Rate Limiting', () => {
    ...
    it('should allow submission after cooldown expires', () => {
      ...
      localStorage.setItem('unotop:submission-tracker', JSON.stringify({
        count: 1,
        resetDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(), // Next month
    ...
    it('should reset counter at start of next month', () => {
      const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(); // Yesterday
    ...
    it('should track submissions correctly in localStorage', () => {
      ...
      const data = JSON.parse(localStorage.getItem('unotop:submission-tracker') || '{}');
  ...
  describe('LocalStorage Validation (Anti-Poisoning)', () => {
    ...
    it('should log warnings for invalid data', () => {
      ...
      writeV3({
        profile: { lumpSumEur: 20_000_000, monthly: 200_000, horizonYears: 100 }
      });
