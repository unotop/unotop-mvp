...
describe("Debts – KPI and reason line", () => {
  it("shows KPI chip with formatted euro sum", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Mesačné splátky spolu:|Celkové splátky:/i)
      ).toBeTruthy();
  ...
  it("shows reason line under verdict", async () => {
    ...
    // Použi getAllByRole a vyber prvý debt row
    const principals = screen.getAllByRole("spinbutton", {
      name: /Zostatok dlhu/i,
    });
    const rates = screen.getAllByRole("spinbutton", {
      name: /Úrok p\.a\. dlhu/i,
    });
    const payments = screen.getAllByRole("spinbutton", {
      name: /Splátka dlhu/i,
    });
    const years = screen.getAllByRole("spinbutton", {
      name: /Zostáva rokov dlhu/i,
    });
