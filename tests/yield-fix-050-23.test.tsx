...
describe('Yield fix @ 0/50/23', () => {
  ...
  it('Vyvážený has dyn > 0% at 0/50/23', () => {
    const result = getAdjustedMix(balanced.mix, { ...profile, riskPref: 'vyvazeny' });
    const dyn = result.mix.find(m => m.key === 'dyn')?.pct || 0;
    const yieldAnnual = approxYieldAnnualFromMix(result.mix);

    console.log(`[Balanced] dyn=${dyn.toFixed(1)}%, yield=${(yieldAnnual * 100).toFixed(2)}%`);
    console.log(`  Mix:`, result.mix.filter(m => m.pct > 0).map(m => `${m.key}:${m.pct.toFixed(1)}%`).join(', '));
  ...
  it('Rastový has dyn > 0% at 0/50/23', () => {
    const result = getAdjustedMix(growth.mix, { ...profile, riskPref: 'rastovy' });
    const dyn = result.mix.find(m => m.key === 'dyn')?.pct || 0;
    const yieldAnnual = approxYieldAnnualFromMix(result.mix);

    console.log(`[Growth] dyn=${dyn.toFixed(1)}%, yield=${(yieldAnnual * 100).toFixed(2)}%`);
    console.log(`  Mix:`, result.mix.filter(m => m.pct > 0).map(m => `${m.key}:${m.pct.toFixed(1)}%`).join(', '));
  ...
  it('Rastový yield > Vyvážený yield at 0/50/23', () => {
    const balancedResult = getAdjustedMix(balanced.mix, { ...profile, riskPref: 'vyvazeny' });
    const growthResult = getAdjustedMix(growth.mix, { ...profile, riskPref: 'rastovy' });
    ...
    console.log(`Balanced ${(balancedYield * 100).toFixed(2)}% vs Growth ${(growthYield * 100).toFixed(2)}%`);
